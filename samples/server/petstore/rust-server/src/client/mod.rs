            ...
            match response.status().as_u16() {
                200 => {
                    ...
                    Box::new(
                        ...
                        .map(move |body|
                            LoginUserResponse::SuccessfulOperation{ body: body, x_rate_limit: response_x_rate_limit }
                        )
                ...
                201 => {
                    ...
                    let response_x_rate_limit = match response.headers().get::<ResponseXRateLimit>() {
                        Some(response_x_rate_limit) => response_x_rate_limit.0.clone(),
                        None => return Box::new(future::err(ApiError(String::from("Required response header X-Rate-Limit for response 201 was not found.")))) as Box<Future<Item=_, Error=_>>,
                    };
                    let body = response.body();
                    Box::new(
                        ...
                        .map(move |body|
                            LoginUserResponse::Wtf{ body: body, x_rate_limit: response_x_rate_limit }
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                400 => {
                    ...
                    let response_x_rate_limit = match response.headers().get::<ResponseXRateLimit>() {
                        Some(response_x_rate_limit) => response_x_rate_limit.0.clone(),
                        None => return Box::new(future::err(ApiError(String::from("Required response header X-Rate-Limit for response 400 was not found.")))) as Box<Future<Item=_, Error=_>>,
                    };
                    let body = response.body();
                    Box::new(

                        future::ok(
                            LoginUserResponse::InvalidUsername{ x_rate_limit: response_x_rate_limit }
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                code => {
