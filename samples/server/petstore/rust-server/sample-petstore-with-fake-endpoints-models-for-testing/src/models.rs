...
#[derive(Debug, Clone, PartialEq, PartialOrd, Serialize, Deserialize)]
#[serde(rename = "group")]
pub struct XmlObject(object);

impl ::std::convert::From<object> for XmlObject {
    fn from(x: object) -> Self {
        XmlObject(x)
    }
}

impl ::std::convert::From<XmlObject> for object {
    fn from(x: XmlObject) -> Self {
        x.0
    }
}

impl ::std::ops::Deref for XmlObject {
    type Target = object;
    fn deref(&self) -> &object {
        &self.0
    }
...
impl ::std::ops::DerefMut for XmlObject {
    fn deref_mut(&mut self) -> &mut object {
        &mut self.0
    }
}
