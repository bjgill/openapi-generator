...
- API version: 1.0.0
- Build date: 2018-06-12T11:40:52.535+01:00[Europe/London]

This autogenerated project defines an API crate `simple` which contains:
* An `Api` trait defining the API in Rust.
* Data types representing the underlying data model.
* A `Client` type which implements `Api` and issues HTTP requests for each operation.
* A router which accepts HTTP requests and invokes the appropriate `Api` method for each operation.

It also contains an example server and client which make use of `simple`:
* The example server starts up a web server using the `simple` router,
...
* The example client provides a CLI which lets you invoke any single operation on the
  `simple` client by passing appropriate arguments on the command line.
...
* Insert `simple` into the `members` array under [workspace] in the root `Cargo.toml`, e.g., `members = [ "simple" ]`.
* Add `simple = {version = "1.0.0", path = "simple"}` under `[dependencies]` in the root `Cargo.toml`.
* Copy the `[dependencies]` and `[dev-dependencies]` from `simple/Cargo.toml` into the root `Cargo.toml`'s `[dependencies]` section.
...
```
cp simple/examples/server.rs src/main.rs
cp simple/examples/server_lib/mod.rs src/lib.rs
cp simple/examples/server_lib/server.rs src/server.rs
```
